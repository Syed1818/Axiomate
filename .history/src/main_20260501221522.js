...
cypherLetters.forEach((letter, index) => {
  let iterations = 0;
  // Stagger the locking left-to-right
  const maxIterations = 40 + index * 12 + Math.random() * 10;
  ...
  }, 30); // 30ms per frame = ultra smooth high-framerate matrix text
});
...
const minPreloaderTime = 4800; // extended for a smooth, unhurried cinematic effect
