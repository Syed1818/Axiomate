...
function initProcessCards() {
  ...
  gsap.utils.toArray('.timeline-item').forEach((item) => {
    ...
    if (dot) tl.from(dot, { scale: 0, opacity: 0, duration: 0.4, ease: 'back.out(2)' });
    
    if (content) {
      tl.from(content, { 
        opacity: 0, 
        x: item.classList.contains('right-align') ? 50 : -50,
        filter: 'blur(8px)',
        duration: 0.8, 
        ease: 'power3.out' 
      }, '-=0.2');
  ...
  gsap.utils.toArray('.process-cards').forEach(grid => {
    gsap.from(grid.querySelectorAll('.process-card'), {
      scrollTrigger: { trigger: grid, start: 'top 80%' },
      opacity: 0, y: 40, filter: 'blur(8px)', duration: 0.8, stagger: 0.15, ease: 'power3.out'
    });
  });
}
...
function initContactForm() {
  gsap.utils.toArray('.contact-form').forEach(form => {
    ...
    gsap.from(inputs, {
      ...
      opacity: 0, x: -20, filter: 'blur(5px)', duration: 0.6, stagger: 0.1, ease: 'power2.out'
    });
  });
}
