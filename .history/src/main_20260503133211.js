...
import { startMatrixEffect, startTerminalBoot } from './preloader.js';
...
function initRouter() {
  ...
  if (enterBtn) {
    ...
    try {
      ...
      // Remove current content sections safely
      document.querySelectorAll('section, .marquee-strip, .brand-ticker, footer').forEach(el => el.remove());

      // Insert new content
      const newContent = doc.querySelectorAll('section, .marquee-strip, .brand-ticker, footer');
      ...
      // Give browser a tick to reflow DOM before GSAP calculates heights
      setTimeout(() => {
        refreshAnimations();
        initAnimations();
        initTiltCards();
...
function initFormHandler() {
  ...
  initRouter(lenis, initSmoothLinks);
});
