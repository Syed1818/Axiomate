  ...
  const logoMat = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        vec3 baseColor = vec3(1.0, 0.2, 0.4); // Crimson
        vec3 pulseColor = vec3(1.0, 0.8, 0.9); // Hot white-pink when speaking
        vec3 color = mix(baseColor, pulseColor, uAudioPulse * 1.5);
        
        gl_FragColor = vec4(color * fresnel * scanline * uIntensity, (fresnel * 0.8 + 0.2) * scanline * uIntensity);
      }
    `,
    transparent: true,
