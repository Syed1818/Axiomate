...
export function startHolographicForgePreloader(canvas, onComplete, fastMode = false) {
  ...
  // Advanced Lighting for Dark Polished Metal
  scene.add(new THREE.AmbientLight(0xffffff, 0.4));
  const dirLight = new THREE.DirectionalLight(0xffaa00, 3.0); // Warm orange rim light
  dirLight.position.set(10, 20, 15);
  scene.add(dirLight);

  const dirLight2 = new THREE.DirectionalLight(0xff3366, 2.0); // Crimson fill light
  ...
  // Glowing Torus Ring
  const ringGeo = new THREE.TorusGeometry(18, 0.2, 64, 128); // Thinner, smoother ring
  const ringMat = new THREE.MeshStandardMaterial({
    color: 0x000000, emissive: 0xff3366, emissiveIntensity: 3.0, transparent: true
  });
  const ring = new THREE.Mesh(ringGeo, ringMat);
  ring.rotation.x = -Math.PI / 2;
  ring.position.y = -8;
  ring.scale.set(0.01, 0.01, 0.01); // Start hidden for intro animation
  forgeGroup.add(ring);

  // Volumetric Light Cone Custom Shader
  const coneGeo = new THREE.CylinderGeometry(20, 12, 30, 64, 1, true);
  const coneMat = new THREE.ShaderMaterial({
    uniforms: { uOpacity: { value: 0.0 } }, // Start hidden for intro animation
    vertexShader: `
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        // Crimson volumetric light beam
        gl_FragColor = vec4(1.0, 0.2, 0.4, light * 0.25 * uOpacity);
      }
    `,
  ...
  fontLoader.load('https://unpkg.com/three@0.160.0/examples/fonts/helvetiker_bold.typeface.json', (font) => {
    const textGeo = new TextGeometry('AXIOMATE', {
      font: font, size: 3.8, height: 0.8, curveSegments: 24, // Sleeker size, much smoother curves
      bevelEnabled: true, bevelThickness: 0.08, bevelSize: 0.04, bevelSegments: 8 // Sharper, premium bevels
    });
    ...
    const particleMat = new THREE.ShaderMaterial({
      uniforms: {
        ...
        uColor: { value: new THREE.Color(0xff3366) }, // Crimson particles
        uOpacity: { value: 1.0 }
      },
      vertexShader: `
        varying float vY;
        void main() {
          vY = position.y;
          gl_PointSize = 2.5; // Finer, more detailed hologram particles
          gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
        }
      `,
      fragmentShader: `
        ...
        void main() {
          ...
          // Heat gradient: white-hot at the printing edge, cooling to crimson below
          float glow = smoothstep(uPrintProgress - 1.5, uPrintProgress, vY);
          vec3 finalColor = mix(uColor * 0.6, vec3(1.0, 0.8, 0.8), glow); 
          
          gl_FragColor = vec4(finalColor, uOpacity * (1.0 - dist * 1.5));
        }
      `,
    ...
    const solidMat = new THREE.MeshStandardMaterial({
      color: 0x111111, metalness: 1.0, roughness: 0.15, transparent: true, opacity: 0.0 // Dark polished metal
    });
    const solidMesh = new THREE.Mesh(textGeo, solidMat);
    textGroup.add(solidMesh);

    // Act II: The Scanner
    const scannerGeo = new THREE.PlaneGeometry(45, 45);
    const scannerMat = new THREE.MeshBasicMaterial({
      color: 0xffaa00, transparent: true, opacity: 0.0, blending: THREE.AdditiveBlending, side: THREE.DoubleSide // Orange scanning laser
    });
    ...
    // Seq 0: The Forge Powers Up (Establishing Shot)
    if (fastMode) {
      tl.timeScale(8);
      camera.position.set(0, 2, 45);
      ring.scale.set(1, 1, 1);
      coneMat.uniforms.uOpacity.value = 0.6;
    } else {
      camera.position.set(0, 10, 60); // Start higher and further back
      tl.to(camera.position, { y: 2, z: 45, duration: 2.5, ease: "power3.out" }, 0)
        .to(ring.scale, { x: 1, y: 1, z: 1, duration: 2.0, ease: "elastic.out(1, 0.5)" }, 0.2)
        .to(coneMat.uniforms.uOpacity, { value: 0.6, duration: 1.5, ease: "power2.inOut" }, 0.5);
    }

    // Seq 1: 3D Print the particles upward
    tl.to(particleMat.uniforms.uPrintProgress, { value: maxY + 0.5, duration: 3.5, ease: "power1.inOut" }, fastMode ? 0 : "-=0.5")
