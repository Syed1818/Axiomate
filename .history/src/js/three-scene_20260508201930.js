...
export function initGlobalScene(canvasId) {
  ...
  // ── GSAP Synchronized Events ─────────────────────────────────────────────
  window.addEventListener('scrollSceneUpdate', (e) => {
    state.scroll = e.detail.progress;
  });

  window.addEventListener('morphScene', (e) => {
    const { shape, palette, label } = e.detail;
    const idx = SECTIONS.findIndex(s => s.shape === shape);
    ...
    if (idx !== -1 && idx !== currentSectionIndex) {
      ...
      if (pal) {
        ...
        hudGroup.children.forEach((ring, i) => {
          ring.material.color = pal[i % 3];
          ring.material.needsUpdate = true;
        });
      }

      // Update section indicator
      updateSectionIndicator(label, idx);
    }
  });
  ...
  function updateThemeSettings(theme) {
    ...
    // Trigger color palette switch
    const activePalettes = isLight ? LIGHT_PALETTES : PALETTES;
    const pal = activePalettes[targetPalette] || activePalettes.hero;
    for (let i = 0; i < COUNT; i++) {
      const c = pal[i % pal.length];
      nextColors[i * 3]     = c.r;
      nextColors[i * 3 + 1] = c.g;
    ...
    colorTransitionTimer = 0; // Trigger the blending lerp
