...
// ===== PRELOADER — CINEMATIC CYPHER =====
const cypherLetters = document.querySelectorAll('.pl-letter.cypher');
const cypherChars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789@#$%&*<>";

function decodeWord(word, onComplete) {
  let completed = 0;
  cypherLetters.forEach((letter, index) => {
    let iterations = 0;
    // Fast decipher for first few letters, slightly longer for the last ones
    const maxIterations = 15 + index * 8 + Math.floor(Math.random() * 8);
    ...
    const interval = setInterval(() => {
      if (iterations >= maxIterations) {
        ...
        completed++;
        if (completed === cypherLetters.length && onComplete) onComplete();
      } else {
        letter.textContent = cypherChars[Math.floor(Math.random() * cypherChars.length)];
        iterations++;
      }
    }, 30);
  });
...
// Phase 1: AXIOMATE -> Hold for 1.2s -> Phase 2: WELCOME!
decodeWord("AXIOMATE", () => {
  setTimeout(() => {
    decodeWord("WELCOME!", () => {});
  }, 1200); 
...
if (textEl) {
  ...
  }, 1100); // 5 messages synced to the ~6 second boot sequence
}

// Dismiss preloader safely after load, ensuring min display time
const minPreloaderTime = 6300; // Extended to allow AXIOMATE and WELCOME sequences to finish
